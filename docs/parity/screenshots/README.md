...
Keep every remaining flag **false** until the matching row is checked with PNG evidence.

Tessera proofs live in `.github/res/Tessera/{StyleId}.png` (not this folder).

Archive references: [Jax-Core](https://github.com/Jax-Core) module READMEs and [legacy/](../../legacy/).

| Module    | Flagship style        | Host surface         | Jax-Core reference | Checklist                                                                            |
| --------- | --------------------- | -------------------- | ------------------ | ------------------------------------------------------------------------------------ |
| Tessera   | Material You + Fluent | `TesseraLayouts`     | YourFlyouts        | [x] All 11 styles in `.github/res/Tessera/{StyleId}.png` (`tessera_layout_fidelity`) |
| Chrono    | Square                | `ChronoStyleFactory` | ModularClocks      | [ ] Arc decoration [ ] Date block typography [ ] Text/Tech chrome variants           |
| Phono     | Compact               | `PhonoStyleFactory`  | ModularPlayers     | [ ] Art + transport horizontal [ ] Square/Windows11 vertical presets                 |
...
| Inlay     | Windows11             | `InlayTileView`      | ValliStart         | [ ] Two-column pins [ ] Catalog search                                               |
| Chord     | Square                | `ChordTileView`      | Keylaunch          | [ ] Action list [ ] Animation presets                                                |
| Substrate | DEFAULT               | `SubstrateTileView`  | MIUI-Shade         | [ ] SMTC mini-tile [ ] Quick settings grid                                           |
| Slate     | Square                | `SlateTileView`      | IdleStyle          | [ ] Idle date hint [ ] Custom media deferred                                         |
