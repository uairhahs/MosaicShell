...
namespace MosaicShell.Core.Tests
{
    ...
    public class HubParityBacklogTests
    {
        ...
        {
            _ = capability.Should().NotBeNullOrWhiteSpace();
            if (!implemented)
            {
            ...
            if (CompanionProof.TryGetValue(capability, out string? proof))
            {
                _ = proof.Should().NotBeNullOrWhiteSpace($"true flag {capability} needs companion proof");
            }
        }
        ...
        {
            _ = ModuleCatalog.All.Should().HaveCount(10);
        }

        [Fact]
        public void Oversold_mvp_flags_are_false_until_bars_met()
        {
            Dictionary<string, bool> map = HubCapabilities.ToDictionary(r => (string)r[0], r => (bool)r[1]);
            _ = map["tile_inlay_mvp"].Should().BeTrue();
            _ = map["tile_chord_mvp"].Should().BeTrue();
            _ = map["tile_substrate_mvp"].Should().BeTrue();
            _ = map["tile_slate_mvp"].Should().BeTrue();
            _ = map["tessera_layout_fidelity"].Should().BeTrue();
            _ = map["tessera_os_acrylic_win11_eval"].Should().BeTrue();
            ...
            _ = map["tile_tessera_mvp"].Should().BeTrue();
            _ = map["tile_chrono_mvp"].Should().BeTrue();
            _ = map["tile_phono_mvp"].Should().BeTrue();
            _ = map["tile_pulse_mvp"].Should().BeTrue();
            _ = map["tile_canvas_mvp"].Should().BeTrue();
            _ = map["chrono_layout_fidelity"].Should().BeFalse();
            _ = map["phono_layout_fidelity"].Should().BeFalse();
            _ = map["pulse_layout_fidelity"].Should().BeFalse();
            _ = map["canvas_layout_fidelity"].Should().BeFalse();
            _ = map["mixdeck_layout_fidelity"].Should().BeFalse();
            _ = map["inlay_layout_fidelity"].Should().BeFalse();
            _ = map["chord_layout_fidelity"].Should().BeFalse();
            _ = map["substrate_layout_fidelity"].Should().BeFalse();
            _ = map["slate_layout_fidelity"].Should().BeFalse();
        }
    }
    ...
    public class HonestyGateTests
    {
        ...
        {
            _ = StyleCatalog.IdsFor("Mixdeck").Should().Contain("Fluent");
            _ = typeof(IAppAudioService).GetMethod(nameof(IAppAudioService.SetMuted)).Should().NotBeNull();
            _ = typeof(IAppAudioService).GetMethod(nameof(IAppAudioService.SetVolume)).Should().NotBeNull();
            _ = typeof(IHostUiBridge).GetMethod(nameof(IHostUiBridge.OpenOverlayAsync)).Should().NotBeNull();
        }

        [Fact]
        public void Inlay_mvp_bar_documented_and_catalog_behavior_proven()
        {
            _ = StyleCatalog.IdsFor("Inlay").Should().Contain("Windows11");
            _ = typeof(InlaySettings).GetProperty(nameof(InlaySettings.Pins)).Should().NotBeNull();
            _ = LaunchTargetCatalog.Search("notepad").Should().NotBeEmpty();
            _ = typeof(IHostUiBridge).GetMethod(nameof(IHostUiBridge.OpenOverlayAsync)).Should().NotBeNull();
        }
        ...
        {
            _ = StyleCatalog.IdsFor("Chord").Should().Contain("Square");
            _ = typeof(ChordSettings).GetProperty(nameof(ChordSettings.Actions)).Should().NotBeNull();
            _ = typeof(IHostUiBridge).GetMethod(nameof(IHostUiBridge.OpenOverlayAsync)).Should().NotBeNull();
        }
        ...
        {
            _ = StyleCatalog.IdsFor("Substrate").Should().Contain("DEFAULT");
            _ = typeof(IAudioService).GetProperty(nameof(IAudioService.IsMuted)).Should().NotBeNull();
            _ = typeof(SubstrateSettings).GetProperty(nameof(SubstrateSettings.ShowMute)).Should().NotBeNull();
            _ = typeof(IHostUiBridge).GetMethod(nameof(IHostUiBridge.OpenOverlayAsync)).Should().NotBeNull();
        }
        ...
        {
            _ = StyleCatalog.IdsFor("Slate").Should().Contain("Square");
            _ = typeof(IIdleService).GetEvent(nameof(IIdleService.IdleThresholdReached)).Should().NotBeNull();
            _ = typeof(IFullscreenProbe).GetProperty(nameof(IFullscreenProbe.IsForegroundFullscreen)).Should().NotBeNull();
            _ = typeof(SlateSettings).GetProperty(nameof(SlateSettings.HideOnFullscreen)).Should().NotBeNull();
            _ = typeof(IHostUiBridge).GetMethod(nameof(IHostUiBridge.OpenOverlayAsync)).Should().NotBeNull();
            _ = typeof(IHostUiBridge).GetMethod(nameof(IHostUiBridge.CloseOverlay)).Should().NotBeNull();
        }

        [Fact]
        public void Hotkey_caps_register_in_catalog()
        {
            foreach (string? id in new[] { "Inlay", "Chord", "Substrate", "Slate", "Mixdeck" })
            {
                _ = ModuleCatalog.IsCapability(id).Should().BeTrue(id);
            }
        }
        ...
        {
            foreach (string? id in new[] { "Chrono", "Phono", "Pulse", "Canvas" })
            {
                _ = ModuleCatalog.TryGet(id, out ModuleInfo? info).Should().BeTrue(id);
                _ = info!.Kind.Should().Be(ModuleKind.Widget, id);
                _ = StyleCatalog.IdsFor(id).Should().NotBeEmpty(id);
            }
        }
        ...
        public void Widget_mvp_bars_documented_and_services_exist()
        {
            ...
            _ = typeof(ISystemMetricsService).GetMethod(nameof(ISystemMetricsService.Sample)).Should().NotBeNull();
            _ = typeof(IMediaSessionService).GetMethod(nameof(IMediaSessionService.PlayPauseAsync)).Should().NotBeNull();
            _ = typeof(IAudioLevelService).GetProperty(nameof(IAudioLevelService.Bands)).Should().NotBeNull();
            _ = StyleCatalog.IdsFor("Chrono").Should().Contain("Square");
            _ = StyleCatalog.IdsFor("Phono").Should().Contain("Compact");
            _ = StyleCatalog.IdsFor("Pulse").Should().Contain("Regular");
            _ = StyleCatalog.IdsFor("Canvas").Should().Contain("DEFAULT");
            _ = ModuleInstaller.IsNativeModuleStub(
        ...
        private static string FindRepoTiles()
        {
            ...
            {
                string tiles = Path.Combine(dir.FullName, "Tiles");
