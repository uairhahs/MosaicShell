...
public class MediaLikePolicyTests
{
    ...
    public void ShouldShowLikedHeart(int? rating, bool expected) =>
        MediaLikePolicy.ShouldShowLikedHeart(rating).Should().Be(expected);
    ...
    public void MaySendUnlikeRating_only_when_host_knows_liked(int hostRating, bool expected) =>
        MediaLikePolicy.MaySendUnlikeRating(hostRating).Should().Be(expected);

    [Fact]
    public void LikeRequestRating_is_five() =>
        MediaLikePolicy.DefaultLikeRequestRating.Should().Be(5);
    ...
    public void ResolveLikeRequestRating_matches_player_semantics(
        ...
        int hostRating,
        int expected) =>
        MediaLikePolicy.ResolveLikeRequestRating(player, wantLiked, hostRating).Should().Be(expected);
    ...
    public void ShouldShowDislikedThumb(int? rating, bool expected) =>
        MediaLikePolicy.ShouldShowDislikedThumb(rating).Should().Be(expected);
    ...
    public void SupportsDislike_for_youtube_music_only(string? appId, string? playerName, bool expected) =>
        MediaLikePolicy.SupportsDislike(appId, playerName).Should().Be(expected);
    ...
    public void MaySendUndislikeRating_only_when_host_knows_disliked(int hostRating, bool expected) =>
        MediaLikePolicy.MaySendUndislikeRating(hostRating).Should().Be(expected);
    ...
    public void ResolveDislikeRequestRating_matches_player_semantics(
        ...
        int hostRating,
        int expected) =>
        MediaLikePolicy.ResolveDislikeRequestRating(player, wantDisliked, hostRating).Should().Be(expected);
}
...
public class CompositeMediaLikeRatingTests
{
    ...
    public void Merge_attaches_wnp_rating_for_browser_sessions()
    {
        ...
        merged!.LikeRating.Should().Be(5);
    }

    [Fact]
    public void Merge_omits_like_rating_for_native_smtc_only()
    {
        var smtc = new MediaSessionInfo("Song", "Artist", "Spotify.exe", true, null, 10, 180);

        var merged = CompositeMediaSessionService.Merge(smtc, wnp: null);

        merged!.LikeRating.Should().BeNull();
    }
}
