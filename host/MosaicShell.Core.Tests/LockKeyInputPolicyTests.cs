...
namespace MosaicShell.Core.Tests
{
    public class LockKeyInputPolicyTests
    {
        ...
        {
            _ = LockKeyInputPolicy.IsToggleVirtualKey(vk).Should().Be(expected);
        }
        ...
        {
            _ = LockKeyInputPolicy.ShouldSampleFromHook(nCode, msg, vk).Should().Be(expected);
        }
        ...
        {
            _ = LockKeyInputPolicy.IsToggleBitOn(state).Should().Be(expected);
        }
        ...
        {
            _ = LockKeyInputPolicy.IsToggleKeyDown(msg).Should().Be(expected);
        }

        [Fact]
        public void KindFromVirtualKey_maps_toggle_keys()
        {
            _ = LockKeyInputPolicy.KindFromVirtualKey(0x14).Should().Be(LockKeyKind.CapsLock);
            _ = LockKeyInputPolicy.KindFromVirtualKey(0x90).Should().Be(LockKeyKind.NumLock);
            _ = LockKeyInputPolicy.KindFromVirtualKey(0x91).Should().Be(LockKeyKind.ScrollLock);
            _ = LockKeyInputPolicy.KindFromVirtualKey(0x41).Should().BeNull();
        }
        ...
        public void Lock_keys_use_poll_policy()
        {
            ...
            _ = LockKeyInputPolicy.MustEdgeToggleOnKeyDown.Should().BeTrue();
        }
    }
}
