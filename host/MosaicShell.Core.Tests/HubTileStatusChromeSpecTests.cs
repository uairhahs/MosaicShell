...
namespace MosaicShell.Core.Tests
{
    public class HubTileStatusChromeSpecTests
    {
        ...
        {
            _ = HubTileStatusChromeSpec.Resolve(installed, capability, armed, running).Should().Be(expected);
        }
        ...
        {
            _ = HubTileStatusChromeSpec.HexFor(kind).Should().Be(hex);
            _ = TesseraAccentColor.TryParse(hex, out _, out _, out _).Should().BeTrue();
        }
        ...
        public void Capability_running_flag_does_not_override_armed_state()
        {
            ...
            _ = HubTileStatusChromeSpec.Resolve(installed: true, isCapability: true, armed: true, running: true)
