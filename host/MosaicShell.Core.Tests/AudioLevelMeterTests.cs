...
public class AudioLevelMeterTests
{
    ...
    {
        AudioLevelMeter.BandCount.Should().Be(16);
    }

    [Fact]
    public void Silent_float_buffer_yields_zero_peak_and_bands()
    {
        var silence = new byte[64]; // 16 float zeros
        Span<float> bands = stackalloc float[AudioLevelMeter.BandCount];
        for (var i = 0; i < bands.Length; i++)
            bands[i] = 0.5f; // should be overwritten

        AudioLevelMeter.ProcessIeeeFloat(silence, bands, out var peak);

        peak.Should().Be(0);
        bands.ToArray().Should().OnlyContain(b => b == 0);
    }
    ...
    public void Loud_samples_clamp_peak_and_bands_to_one()
    {
        ...
        var buffer = MemoryMarshal.AsBytes(samples.AsSpan()).ToArray();
        Span<float> bands = stackalloc float[AudioLevelMeter.BandCount];
        ...
        foreach (var b in bands)
            b.Should().BeInRange(0, 1);
        bands.ToArray().Should().Contain(1f);
    }
    ...
    {
        Span<float> bands = stackalloc float[AudioLevelMeter.BandCount];
        AudioLevelMeter.ProcessIeeeFloat(stackalloc byte[2], bands, out var peak);
        peak.Should().Be(0);
    }
    ...
    {
        var field = typeof(WindowsAudioLevelService)
            .GetFields(BindingFlags.Instance | BindingFlags.NonPublic)
            .Single(f => f.Name == "_capture");

        field.FieldType.Should().Be(typeof(WasapiRecorder),
            "NAudio 3 deprecates WasapiLoopbackCapture; Pulse metering must use WasapiRecorder");
        field.FieldType.Name.Should().NotBe("WasapiLoopbackCapture");
    }
}
