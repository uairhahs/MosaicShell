...
namespace MosaicShell.Core.Tests
{
    public class StyleIdsTests
    {
        ...
        {
            _ = StyleIds.Normalize(input).Should().Be(expected);
        }
        ...
        {
            _ = StyleIds.DisplayName(input).Should().Be(expected);
        }
        ...
        {
            TesseraSettings settings = new() { Style = input };
            _ = StyleIds.TryMigratePersistedStyle(settings).Should().Be(changed);
            _ = settings.Style.Should().Be(expected);
        }
    }
}
