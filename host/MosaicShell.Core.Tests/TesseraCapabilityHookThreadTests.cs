...
namespace MosaicShell.Core.Tests
{
    public class TesseraCapabilityHookThreadTests : IDisposable
    {
        ...
        public async Task Arm_starts_legacy_hook_on_host_thread()
        {
            ...
            BridgeUi bridge = new(new NullFlyoutPresenter());
            TrackingLockKeysService lockKeys = new();
            TrackingLegacyHook legacy = new(() => bridge.InHostThread);
            HostServices services = CreateServices(lockKeys, legacy);
            TesseraCapability cap = new(TestCapabilityContext.Create(services, bridge));

            await cap.ArmAsync();

            _ = lockKeys.StartCalled.Should().BeTrue();
            _ = legacy.StartOnHostThread.Should().BeTrue();
            await cap.DisarmAsync();
            _ = legacy.StopOnHostThread.Should().BeTrue();
            _ = lockKeys.StopCalled.Should().BeTrue();
        }
        ...
        internal sealed class TrackingLegacyHook(Func<bool>? isHostThread = null) : ILegacyMediaKeyHook
        {
            private readonly Func<bool> _isHostThread = isHostThread ?? (() => false);
