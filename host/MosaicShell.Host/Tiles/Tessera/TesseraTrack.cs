...
namespace MosaicShell.Host.Tiles.Tessera
{
    ...
    public sealed class TesseraTrack : Panel
    {
        ...
        {
            _userUntil = DateTime.UtcNow.AddMilliseconds(350);
        }
        ...
        public void SetValueSilent(double v)
        {
            ...
            double clamped = VolumePercent.Quantize(v);
            _suppress = true;
            try
            {
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            if (IsVertical)
            {
                double w = double.IsInfinity(availableSize.Width) ? 28 : availableSize.Width;
            ...
            double hw = double.IsInfinity(availableSize.Width)
                ? (!double.IsNaN(Width) && Width > 0 ? Width : 200)
                : availableSize.Width;
            double hh = double.IsInfinity(availableSize.Height) ? 28 : Math.Min(availableSize.Height, 28);
            if (!double.IsNaN(Width) && Width > 0)
            {
                hw = Width;
            }
        ...
        {
            double pad = Math.Max(0, TrackPad);
            double thickness = Math.Max(2, TrackThickness);
            double v = Math.Clamp(Value, 0, 1);
            double w = finalSize.Width;
            double h = finalSize.Height;
            double thumb = FatThumb ? 22.0 : 14.0;
            if (FatThumb && IsVertical)
            {
                thumb = Math.Max(28, w - 4);
            }
            ...
            if (IsVertical)
            {
                ...
                if (!FatThumb && ShellRadius > 0.5 && thickness >= w * 0.85)
                {
                    ...
                    double trackLen = Math.Max(1, h - (pad * 2));
                    IBrush accent = AccentBrushOverride ?? TesseraPalette.AccentBrush;
                    bool useM3Handle = GlassFill && ShowThumb;
                    double fillH = ComputeVerticalFillHeight(v, trackLen, useM3Handle);
                    double fillTop = pad + trackLen - fillH;

                    _back.IsVisible = false;
                    ApplyFillBrush();

                    double bottomR = fillH >= trackLen - 0.5 ? r : Math.Min(r, Math.Max(0, fillH / 2));
                    double topR = fillH >= trackLen - 0.5 ? r : (useM3Handle ? M3InsideCorner : 0);
                    CornerRadius fillCorner = new(topR, topR, bottomR, bottomR);
                    ...
                    {
                        double thumbY = Math.Clamp(fillTop - (thumb / 2), 0, Math.Max(0, h - thumb));
                ...
                else if (ExpressiveVertical)
                {
                    ...
                    double shellTopR = Math.Max(0, ShellRadius);
                    double shellBottomR = Math.Max(0, ShellEndRadius);
                    bool shellClip = shellTopR > 0.5 || shellBottomR > 0.5;
                    ApplyShellClip(w, h, shellTopR, shellBottomR);
                    const double bedInsetX = 4.0;
                    double insetY = shellClip ? 0 : Math.Max(6, TrackPad);
                    double bedW = shellClip
                        ? Math.Max(12, w - (bedInsetX * 2))
                        : TrackThickness > 20
                            ? Math.Min(w - (Math.Max(4, TrackPad) * 2), TrackThickness)
                            : Math.Max(12, w - (Math.Max(4, TrackPad) * 2));
                    double bedX = (w - bedW) / 2;
                    double trackLen = Math.Max(1, h - (insetY * 2));
                    double fillH = ComputeVerticalFillHeight(v, trackLen, useM3Handle: true);
                    double fillTop = insetY + trackLen - fillH;
                    IBrush accent = AccentBrushOverride ?? TesseraPalette.AccentBrush;

                    double backTopR = shellClip ? Math.Max(0, shellTopR - bedInsetX) : bedW / 2.0;
                    double backBottomR = shellClip ? shellBottomR : bedW / 2.0;
                    ...
                    double fillTopR = fillH >= trackLen - 0.5 ? backTopR : M3InsideCorner;
                    double fillBottomR = backBottomR;
                ...
                else if (MaterialYouVerticalFill)
                {
                    ...
                    double spine = Math.Max(2, TrackThickness);
                    double fillW = Math.Max(10, w - (pad * 2));
                    double fillX = (w - fillW) / 2;
                    double spineX = (w - spine) / 2;
                    double trackLen = Math.Max(1, h - (pad * 2));
                    double fillH = Math.Max(v > 0.001 ? 6 : 0, trackLen * v);
                    double fillTop = pad + trackLen - fillH;
                ...
                else if (FatThumb)
                {
                    ...
                    double fillH = Math.Max(v > 0.001 ? 8 : 0, trackLen * v);
                    _fill.Arrange(new Rect(2, pad + trackLen - fillH, Math.Max(1, w - 4), fillH));
                    double th = _thumb.Height;
                    double ty = Math.Clamp(pad + trackLen - fillH - (th * 0.35), pad, h - th - pad);
                ...
                else
                {
                    ClipToBounds = false;
                    _back.IsVisible = true;
                    double x = Math.Max(0, (w - thickness) / 2);
                    double trackLen = Math.Max(1, h - (pad * 2));
                    double fillLen = Math.Max(0, trackLen * v);
                    double fillTop = pad + trackLen - fillLen;

                    // Edge-to-edge (Amber): radius matches full pill when thickness ≈ width
                    double radius = pad <= 0.5 ? Math.Min(w, thickness) / 2 : thickness / 2;
                    ...
                    Rect fillRect = new(x, fillTop, thickness, Math.Max(fillLen > 0 ? fillLen : 0, v > 0.001 ? 2 : 0));
            ...
            {
                double y = Math.Max(0, (h - thickness) / 2);
                double trackLen = Math.Max(1, w - (pad * 2));
                double fillLen = Math.Max(0, trackLen * v);
                ...
                double thumbX = Math.Clamp(pad + fillLen - (thumb / 2), 0, Math.Max(0, w - thumb));
        ...
        private void ApplyPointer(Point p)
        {
            ...
            if (IsVertical)
            {
                double len = Math.Max(1, Bounds.Height - (pad * 2));
                raw = Math.Clamp((Bounds.Height - pad - p.Y) / len, 0, 1);
            }
            else
            {
                double len = Math.Max(1, Bounds.Width - (pad * 2));
        ...
        private static double ComputeVerticalFillHeight(double v, double trackLen, bool useM3Handle)
        {
            ...
            double minFill = useM3Handle ? M3HandleThin : 2.0;
        ...
        {
            ClipToBounds = true;
            double clipR = Math.Max(topR, bottomR);
            ...
            double w, double insetY, double fillTop, double fillH, double v, IBrush accent, double bedW)
        {
            ...
            {
                double handleW = Math.Min(w, Math.Max(M3HandleWide, bedW + 12));
                double handleY = Math.Max(insetY, fillTop - M3ThumbGap - M3HandleThin);
