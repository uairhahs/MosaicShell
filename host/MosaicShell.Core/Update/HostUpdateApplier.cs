...
namespace MosaicShell.Core.Update
{
    ...
    public static class HostUpdateApplier
    {
        ...
        {
            string full = Path.GetFullPath(setupExePath);
            if (!File.Exists(full))
            {
                throw new FileNotFoundException("Setup executable not found.", full);
            }

            string args = string.IsNullOrWhiteSpace(extraArgs)
        ...
        {
            ProcessStartInfo psi = CreateSetupStartInfo(setupExePath, extraArgs);
