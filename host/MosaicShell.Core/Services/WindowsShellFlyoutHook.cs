...
namespace MosaicShell.Core.Services
{
    ...
    public sealed class WindowsShellFlyoutHook : IShellFlyoutTriggerSource
    {
        ...
        public void Start()
        {
            ...
            for (int i = 0; i < 50 && _hwnd == IntPtr.Zero && _running; i++)
            {
                Thread.Sleep(10);
            }
        ...
        public void Stop()
        {
            ...
            {
                try { _ = PostMessage(_hwnd, WM_QUIT, IntPtr.Zero, IntPtr.Zero); } catch { /* ignore */ }
            }
            _ = (_thread?.Join(1000));
            _thread = null;
            _hwnd = IntPtr.Zero;
        ...
        private void MessageLoop()
        {
            try
            {
                ...
                string className = "MosaicShell.ShellFlyoutHook." + Guid.NewGuid().ToString("N");
                WNDCLASS wc = new()
                {
                ...
                while (_running)
                {
                    int gm = GetMessage(out MSG msg, IntPtr.Zero, 0, 0);
                    if (gm <= 0)
                    {
                        break;
                    }
                ...
                try { _ = DeregisterShellHookWindow(_hwnd); } catch { /* ignore */ }
                try { _ = DestroyWindow(_hwnd); } catch { /* ignore */ }
                _hwnd = IntPtr.Zero;
            }
            catch (Exception ex)
            {
        ...
        {
            if (msg == _shellHookMsg && ShellFlyoutTriggerDecoder.TryDecode(wParam, lParam, out ShellFlyoutKind kind))
            {
    ...
    public sealed class FakeShellFlyoutTriggerSource : IShellFlyoutTriggerSource
    {
        ...
        public void Raise(ShellFlyoutKind kind)
        {
            Triggered?.Invoke(this, kind);
        }
    }
}
