...
public static class MediaSessionChangePolicy
{
        ...
        bool incomingReportedChange)
    {
        ...
        if (!incomingReportedChange)
        {
            if (incomingSeconds + PlayingPositionJitterSeconds < committedSeconds)
                return committedSeconds;
            return incomingSeconds;
        }

        if (LooksLikeNewTrackPosition(committedSeconds, incomingSeconds))
            return incomingSeconds;
