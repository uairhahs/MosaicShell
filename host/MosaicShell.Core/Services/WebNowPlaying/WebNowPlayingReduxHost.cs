...
namespace MosaicShell.Core.Services.WebNowPlaying
{
    ...
    /// </summary>
    public sealed class WebNowPlayingReduxHost(int port = WebNowPlayingReduxHost.DefaultPort) : IWebNowPlayingService
    {
        ...
        public int ListenPort { get; } = port;
        ...
        public void Start()
        {
            ...
            _diagDir = Path.Combine(
                ...
                "MosaicShell", "wnp");
            try { _ = Directory.CreateDirectory(_diagDir); } catch { /* ignore */ }
        ...
        {
            DateTime until = DateTime.UtcNow + (timeout ?? TimeSpan.FromSeconds(3));
        ...
        public void Dispose()
        {
            ...
            _listener = null;
            foreach (WebSocket s in _sockets.Values)
            {
        ...
        {
            HttpClient http = new() { Timeout = TimeSpan.FromSeconds(12) };
            _ = http.DefaultRequestHeaders.TryAddWithoutValidation(
                "User-Agent",
                "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36");
            _ = http.DefaultRequestHeaders.TryAddWithoutValidation("Accept", "image/avif,image/webp,image/apng,image/*,*/*;q=0.8");
            return http;
        }

        /// <summary>Same truncation as JS <c>DataView.setUint32(0, id, true)</c>.</summary>
        internal static uint ToJsUint32(long id)
        {
        ...
        private async Task HandleHttpAsync(HttpListenerContext ctx)
        {
            try
            {
                ...
                {
                    byte[] bytes = Encoding.UTF8.GetBytes(BuildStatusJson());
                ...
                {
                    byte[]? cover = Active?.CoverPng;
                    if (cover is null || cover.Length < 32)
                    {
        ...
        private async Task HandleClientAsync(HttpListenerContext ctx, CancellationToken ct)
        {
            ...
            try
            {
                ...
                _ = Interlocked.Increment(ref _clients);
                Trace($"client +{clientId[..8]} clients={_clients}");

                byte[] hello = Encoding.UTF8.GetBytes($"ADAPTER_VERSION {AdapterVersion};WNPLIB_REVISION 3");
                await ws.SendAsync(hello, WebSocketMessageType.Text, true, ct);

                while (ws.State == WebSocketState.Open && !ct.IsCancellationRequested)
                {
                    (WebSocketMessageType type, byte[]? payload) = await ReceiveFullAsync(ws, ct);
            ...
            {
                _ = _sockets.TryRemove(clientId, out _);
                _ = Interlocked.Decrement(ref _clients);
        ...
        private void OnBinary(byte[] payload)
        {
            ...
            uint trunc = BitConverter.ToUInt32(payload, 0);
            byte[] img = new byte[payload.Length - 4];
            ...
            MutablePlayer? player = FindByUint32(trunc);
        ...
        private void OnText(string message)
        {
            ...
            {
                Trace($"text {message.Length}b: {message[..200]}...");
            }
            else
            {
                Trace($"text: {message}");
            ...
            if (!int.TryParse(message.AsSpan(0, sp), out int msgType))
            {
                return;
            }

            string rest = message[(sp + 1)..];
            ...
            switch (msgType)
            {
                ...
                case 1:
                    {
                        ...
                        if (!long.TryParse(rest.AsSpan(0, sp2), out long portId))
                        {
                            return;
                        }
                ...
                case 2:
                    {
                        if (!long.TryParse(rest, out long portId))
                        {
                            return;
                        }

                        if (_players.TryRemove(portId, out _))
                        {
                            Publish();
                        }
        ...
        {
            List<string> tokens = SplitFields(fieldBlob);
            MutablePlayer player = _players.GetOrAdd(portId, id => new MutablePlayer { PortId = id });
            ApplyFields(player, tokens);

            uint trunc = ToJsUint32(portId);
            if (_pendingCovers.TryRemove(trunc, out byte[]? pending))
            {
        ...
        {
            foreach (KeyValuePair<long, MutablePlayer> kv in _players)
            {
        ...
        private async Task TryFetchCoverSrcAsync(MutablePlayer player)
        {
            ...
            try
            {
                byte[]? bytes = await DownloadCoverAsync(src);
                if (bytes is null || bytes.Length < 32)
                {
                    return;
                }

                if (!_players.TryGetValue(player.PortId, out MutablePlayer? live))
                {
                    return;
                }

                if (live.CoverPng is { Length: > 32 } && LooksLikeImage(live.CoverPng))
                {
                    return;
                }
        ...
        private static async Task<byte[]?> DownloadCoverAsync(string src)
        {
            ...
            {
                foreach (string url in CoverUrlCandidates(src))
                {
                    try
                    {
        ...
        {
            List<WnpPlayerSnapshot> list = [.. _players.Values.Select(p => p.ToSnapshot())];
            if (list.Count == 0)
            {
                return null;
            }

            List<WnpPlayerSnapshot> withMedia = [.. list.Where(p => !string.IsNullOrEmpty(p.Title) || p.CoverPng is { Length: > 0 })];
            return withMedia.Count == 0
                ? list.FirstOrDefault()
                : withMedia
                .Where(p => p.IsPlaying)
        ...
        {
            if (string.IsNullOrEmpty(_diagDir))
            {
                return;
            }
            ...
            try
            {
                string ext = LooksLikePng(img) ? "png" : LooksLikeJpeg(img) ? "jpg" : "bin";
        ...
        {
            string msg = $"{DateTime.Now:HH:mm:ss.fff} {line}";
            System.Diagnostics.Debug.WriteLine("[WNP] " + line);
            _trace.Enqueue(msg);
            while (_trace.Count > 80)
            {
                _ = _trace.TryDequeue(out _);
            }
        ...
        {
            MutablePlayer? p = ActivePlayer();
            if (p is null)
            {
                return;
            }

            int next = p.Shuffle ? 0 : 1;
            p.Shuffle = next != 0;
        ...
        public async Task TrySetLikeAsync(bool wantLiked)
        {
            ...
            int data = MediaLikePolicy.ResolveLikeRequestRating(p.Name, wantLiked, p.Rating);
            Trace(
        ...
        public async Task TrySetDislikeAsync(bool wantDisliked)
        {
            ...
            int data = MediaLikePolicy.ResolveDislikeRequestRating(p.Name, wantDisliked, p.Rating);
        ...
        {
            WnpPlayerSnapshot? snap = Active;
            return snap is null ? null : _players.TryGetValue(snap.PortId, out MutablePlayer? p) ? p : null;
        }

        private async Task SendEventAsync(long portId, int eventType, int data)
        {
            int eventId = Interlocked.Increment(ref _eventSeq) & 0x1FF;
            // WNPLIB web: "{portId} {eventId} {event} {data}"
            string msg = $"{portId} {eventId} {eventType} {data}";
            byte[] bytes = Encoding.UTF8.GetBytes(msg);
            Trace($"event → {msg}");
            foreach (KeyValuePair<string, WebSocket> kv in _sockets)
            {
                WebSocket ws = kv.Value;
        ...
        {
            string prevTitle = p.Title;
            string prevArtist = p.Artist;
            Set(t, 1, v => p.Name = v);
            Set(t, 2, v => p.Title = v);
        ...
        {
            if (i < t.Count && t[i].Length > 0 && ulong.TryParse(t[i], out ulong v))
            {
                apply(v);
            }
        }
        ...
        {
            return LooksLikePng(b) || LooksLikeJpeg(b) || LooksLikeWebp(b);
        }
        ...
        {
            return b.Length > 12 && b[0] == (byte)'R' && b[1] == (byte)'I' && b[2] == (byte)'F' && b[3] == (byte)'F';
        }

        private static string Magic(byte[] b)
        {
            return LooksLikePng(b)
                ? "png"
                : LooksLikeJpeg(b) ? "jpg" : LooksLikeWebp(b) ? "webp" : BitConverter.ToString(b, 0, Math.Min(4, b.Length));
        }
        ...
        private sealed class MutablePlayer
        {
            ...
            public WnpPlayerSnapshot ToSnapshot()
            {
                return new()
                {
                    PortId = PortId,
