...
namespace MosaicShell.Core.Services
{
    ...
    public sealed class WindowsBrightnessChangeSource : IBrightnessChangeSource
    {
        ...
        private void Tick()
        {
            if (!_running || !_brightness.IsSupported)
            {
                return;
            }

            double v = _brightness.Brightness;
    ...
    public sealed class WindowsNativeOsdSuppressor : INativeOsdSuppressor
    {
        ...
        public void Stop()
        {
            ...
            try
            {
                ...
                {
                    _ = ShowWindowAsync(_hwndDui, SW_RESTORE);
                }
            }
            catch { /* soft-fail */ }
        ...
        private void HideNativeFlyout(bool permanent)
        {
            try
            {
                ...
                if (_hwndDui != IntPtr.Zero)
                {
                    _ = ShowWindowAsync(_hwndDui, SW_MINIMIZE);
                }

                _ = ShowWindowAsync(_hwndHost, SW_HIDE);
                if (permanent)
                {
                    _ = ShowWindowAsync(_hwndHost, SW_MINIMIZE);
                    _ = ShowWindowAsync(_hwndHost, SW_FORCEMINIMIZE);
                }
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"[OsdSuppressor] hide soft-fail: {ex.Message}");
            }
        }
        ...
        private bool TryResolveNativeFlyout()
        {
            ...
            ResolveClassNames(out string? outerClass, out string? outerName, out string? innerClass, out string? innerName);

            for (nint host = IntPtr.Zero;
                 (host = FindWindowEx(IntPtr.Zero, host, outerClass, outerName)) != IntPtr.Zero;)
            {
                nint dui = FindWindowEx(host, IntPtr.Zero, innerClass, string.IsNullOrEmpty(innerName) ? null : innerName);
                if (dui == IntPtr.Zero)
                {
                    continue;
                }

                _ = GetWindowThreadProcessId(host, out uint pid);
                if (pid == 0)
                {
                    continue;
                }
                ...
                if (!GetWindowBand(host, out nuint band))
                {
                    continue;
                }

                if (band != ZBandAboveLockUx)
                {
                    continue;
                }
        ...
        private static int GetOsBuild()
        {
            try
            {
                string desc = RuntimeInformation.OSDescription; // e.g. Microsoft Windows 10.0.26200
                int dot = desc.LastIndexOf('.');
                if (dot >= 0 && int.TryParse(desc[(dot + 1)..], out int build))
                {
                    return build;
                }
            }
        ...
        private static int GetShellProcessId()
        {
            try
            {
                ...
                _ = GetWindowThreadProcessId(shell, out uint pid);
                return (int)pid;
            }
            catch
            {
        ...
        private void InstallHook()
        {
            ...
            {
                Debug.WriteLine("[OsdSuppressor] SetWinEventHook failed");
            }
        }
        ...
        private void OnWinEvent(IntPtr hWinEventHook, uint eventType, IntPtr hwnd, int idObject, int idChild, uint dwEventThread, uint dwmsEventTime)
        {
            ...
            try
            {
                bool inBurst = DateTimeOffset.UtcNow <= _burstUntil;
                ...
                if (_hwndHost != IntPtr.Zero && hwnd == _hwndHost)
                {
                    switch (eventType)
                    {
                        ...
                        case EVENT_OBJECT_DESTROY:
                            ...
                            _resolved = false;
                            _rehook?.Dispose();
                            _rehook = new Timer(_ => TryRehook(), null, 1500, 3000);
                            break;
                        default:
                            break;
                    }
            ...
            {
                Debug.WriteLine($"[OsdSuppressor] WinEvent soft-fail: {ex.Message}");
            }
        }
    ...
    public sealed class WindowsIdleService : IIdleService
    {
        ...
        public TimeSpan IdleTime
        {
            get
            {
                ...
                int idleMs = unchecked(Environment.TickCount - (int)info.dwTime);
        ...
        public bool IsForegroundFullscreen
        {
            get
            {
                try
                {
                    ...
                    nint mon = MonitorFromWindow(hwnd, 2 /* MONITOR_DEFAULTTONEAREST */);
