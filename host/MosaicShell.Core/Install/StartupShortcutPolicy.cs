...
public static class StartupShortcutPolicy
{
    ...
    {
        var normalized = Path.GetFullPath(exePath).Replace('\\', '/');
        var url = string.IsNullOrWhiteSpace(arguments)
            ? $"file:///{normalized}"
    ...
    public static void WriteHostShortcut(string shortcutPath, string hostExePath, bool trayOnly = false)
    {
        ...
        var args = trayOnly ? HostInstallPolicy.TrayOnlyArg : null;
    ...
    {
        var path = Path.Combine(HostInstallPolicy.StartupDirectory, HostInstallPolicy.StartupShortcutFileName);
        if (File.Exists(path))
            File.Delete(path);
    }
}
