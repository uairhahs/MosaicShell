...
namespace MosaicShell.Core.Capabilities.Ipc
{
    ...
    public sealed class RemoteCapabilityHost : ICapabilityHost
    {
        ...
        {
            return SendRequest(new CapabilityControlMessage(
        ...
        {
            return Task.FromResult(SendRequest(new CapabilityControlMessage(
        ...
        {
            _ = SendRequest(new CapabilityControlMessage(CapabilityControlMessageType.Restore, NextId()));
            return Task.CompletedTask;
        }

        public Task DisarmAllAsync(CancellationToken cancellationToken = default)
        {
            _ = SendRequest(new CapabilityControlMessage(CapabilityControlMessageType.DisarmAll, NextId()));
            return Task.CompletedTask;
        }

        public void Persist()
        {
            _ = SendRequest(new CapabilityControlMessage(CapabilityControlMessageType.Persist, NextId()));
        }
        ...
        private CapabilityControlMessage SendRequest(CapabilityControlMessage request)
        {
            ...
            return conn is null || !conn.IsConnected
        ...
        private sealed class ClientConnection : IDisposable
        {
            ...
            public CapabilityControlMessage SendRequest(CapabilityControlMessage request)
            {
                lock (_gate)
                {
                    ...
                    long deadline = Environment.TickCount64 + CapabilityIpcControlPolicy.ConnectTimeoutMs;
                    while (Environment.TickCount64 < deadline)
                    {
                        if (_responses.TryRemove(request.RequestId, out CapabilityControlMessage? response))
                        {
                            return response;
                        }
