...
namespace MosaicShell.Core.Capabilities.BuiltIn
{
    ...
    public static class HotkeyGestureParser
    {
        ...
        {
            return !string.IsNullOrWhiteSpace(gesture) && OsReserved.Contains(Normalize(gesture));
        }

        public static string Normalize(string gesture)
        {
            return !TryParse(gesture, out ModifierKeys mods, out int vk) ? gesture.Trim() : Format(mods, vk);
        }
        ...
        {
            return !string.IsNullOrWhiteSpace(gesture) && TryParse(gesture, out _, out _) && !IsLikelyOsReserved(gesture)
                ? Normalize(gesture)
                : SafeDefault(moduleId);
        }

        public static string SafeDefault(string moduleId)
        {
        ...
        public static string Format(ModifierKeys mods, int vk)
        {
            ...
            if (mods.HasFlag(ModifierKeys.Win))
            {
                parts.Add("Win");
            }
        ...
        public static string FormatVk(int vk)
        {
            return vk switch
            {
                0x20 => "Space",
