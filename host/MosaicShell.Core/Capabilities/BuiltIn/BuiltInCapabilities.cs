...
public sealed class TesseraCapability : IModuleCapability
{
    ...
    private void OnVolume(object? s, EventArgs e) => Show("vol");
    private void OnBrightness(object? s, EventArgs e) => Show("bright");
    private void OnMedia(object? s, EventArgs e)
    {
        if (_settings.EnableMediaFlyouts) Show("media");
    }
    ...
    {
        _settings = ModuleSettingsStore.Load("Tessera", () => new TesseraSettings());
        _services.OsdSuppressor.SuppressOnce();
        _ui.Flyouts.Show(new FlyoutRequest(
            ModuleId,
            kind,
            _settings.Style,
            _settings.Anchor,
