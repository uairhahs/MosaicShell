...
namespace MosaicShell.Core.Modules.Tessera
{
    ...
    {
        public static bool IsConfigured(string? input)
        {
            return TryParse(input, out _, out _, out _);
        }
        ...
        public static bool TryParse(string? input, out byte r, out byte g, out byte b)
        {
            ...
            if (s.Length != 6
                || !uint.TryParse(s, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out uint rgb))
            {
                return false;
            }
        ...
        {
            return !TryParse(input, out byte r, out byte g, out byte b) ? "" : $"#{r:X2}{g:X2}{b:X2}";
        }
        ...
        {
            return string.Equals(NormalizeOrEmpty(currentHex), NormalizeOrEmpty(preset.Hex), StringComparison.OrdinalIgnoreCase);
        }
    }
