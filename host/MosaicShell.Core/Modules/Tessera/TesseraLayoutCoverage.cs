...
namespace MosaicShell.Core.Modules.Tessera
{
    ...
    public static class TesseraLayoutCoverage
    {
        ...
        public static bool IsPolished(string styleId)
        {
            return Polished.Contains(StyleIds.Normalize(styleId));
        }

        public static bool IsApproximate(string styleId)
        {
            return Approximate.Contains(StyleIds.Normalize(styleId));
        }

        public static bool IsLayoutFidelitySignedOff(string styleId)
        {
            return LayoutFidelitySignedOff.Contains(StyleIds.Normalize(styleId));
        }

        public static bool IsLayoutFidelityDeviated(string styleId)
        {
            return LayoutFidelityDeviated.Contains(StyleIds.Normalize(styleId));
        }
        ...
        {
            IReadOnlyList<string> ids = StyleCatalog.IdsFor("Tessera");
            return ids.Count != 0 && ids.All(id => Polished.Contains(id) || Approximate.Contains(id));
        }

        public static bool CoversLayoutFidelity()
        {
            IReadOnlyList<string> ids = StyleCatalog.IdsFor("Tessera");
            return ids.Count != 0 && ids.All(id => IsLayoutFidelitySignedOff(id) || IsLayoutFidelityDeviated(id));
        }

        /// <summary>
        /// Styles that embed media controls in-layout (no Modern-style stacked strip on volume).
        /// Derived from the in-layout-media capability rather than restating the style id (ADR-0001);
        /// pinned by <c>TesseraFlyoutStyleProfileConsistencyTests.In_layout_media_is_never_also_a_stacked_strip</c>.
        /// </summary>
        public static bool UsesStackedMediaStrip(string styleId)
        {
            return !TesseraFlyoutTweenTargetCatalog.StylePhase2UsesInLayoutMedia(styleId);
        }
        ...
        {
            return StyleIds.Normalize(styleId).Equals(StyleIds.Meter, StringComparison.OrdinalIgnoreCase);
        }
    }
}
