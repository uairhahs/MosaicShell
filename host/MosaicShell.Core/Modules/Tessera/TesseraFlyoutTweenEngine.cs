...
public static class TesseraFlyoutTweenEngine
{
    ...
    {
        var name = TesseraFlyoutAnimationPolicy.NormalizeEase(easeType).ToLowerInvariant();
        return name switch
        {
            "linear" => Linear,
            "insine" => InSine,
    ...
    {
        var tween = new TesseraFlyoutStepTween(durationSteps, 0, 100, easeType);
        var delta = entrance ? 1 : -1;
        for (var i = 0; i < Math.Abs(clock); i++)
            tween.Advance(delta);
        return Math.Clamp(tween.Value / 100.0, 0, 1);
    }

    private static double Linear(double t, double b, double c, double d) => c * t / d + b;

    private static double InQuad(double t, double b, double c, double d) => c * Pow(t / d, 2) + b;
    ...
    private static double InCubic(double t, double b, double c, double d) => c * Pow(t / d, 3) + b;

    private static double OutCubic(double t, double b, double c, double d) => c * (Pow(t / d - 1, 3) + 1) + b;
    ...
    private static double InQuart(double t, double b, double c, double d) => c * Pow(t / d, 4) + b;

    private static double OutQuart(double t, double b, double c, double d) => -c * (Pow(t / d - 1, 4) - 1) + b;
    ...
    private static double InQuint(double t, double b, double c, double d) => c * Pow(t / d, 5) + b;

    private static double OutQuint(double t, double b, double c, double d) => c * (Pow(t / d - 1, 5) + 1) + b;
    ...
    private static double InSine(double t, double b, double c, double d) => -c * Cos(t / d * (Pi / 2)) + c + b;

    private static double OutSine(double t, double b, double c, double d) => c * Sin(t / d * (Pi / 2)) + b;

    private static double InOutSine(double t, double b, double c, double d) => -c / 2 * (Cos(Pi * t / d) - 1) + b;
    ...
    private static double InCirc(double t, double b, double c, double d) => -c * (Sqrt(1 - Pow(t / d, 2)) - 1) + b;

    private static double OutCirc(double t, double b, double c, double d) => c * Sqrt(1 - Pow(t / d - 1, 2)) + b;
    ...
    private static double InBounce(double t, double b, double c, double d) => c - OutBounce(d - t, 0, c, d) + b;
    ...
    private static double InOutElastic(double t, double b, double c, double d)
    {
        ...
        var (p, a, s) = CalculatePas(d, c);
        t -= 1;
        if (t < 0) return -0.5 * (a * Pow(2, 10 * t) * Sin((t * d - s) * (2 * Pi) / p)) + b;
    ...
    private static double Asin(double x) => Math.Asin(x);
}
