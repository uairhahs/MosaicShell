...
namespace MosaicShell.Core.Modules
{
    ...
    public static class ModuleCatalog
    {
        ...
        public static IReadOnlyList<ModuleInfo> All => [.. EnumerateAll()];
        ...
        {
            return TryGet(moduleId, out ModuleInfo? info) && info is not null &&
        ...
        {
            return Directory.Exists(Path.Combine(AppPaths.ModulesDirectory, moduleId));
        }
        ...
        public static IEnumerable<ModuleInfo> EnumerateAll()
        {
            ...
            foreach (ModuleInfo m in BuiltIns)
            {
                _ = seen.Add(m.Id);
                yield return m;
            }

            foreach (ModuleInfo discovered in DiscoverInstalled(seen))
            {
        ...
        private static ModuleInfo? TryFromInstalledManifest(string moduleId)
        {
            ...
            string id = string.IsNullOrWhiteSpace(manifest.Id) ? moduleId : manifest.Id;
            string name = string.IsNullOrWhiteSpace(manifest.DisplayName) ? id : manifest.DisplayName;
            ModuleKind kind = ParseKind(manifest.Kind);
