namespace MosaicShell.Core.Modules
{
    ...
    public static class LaunchTargetCatalog
    {
        ...
        public static IReadOnlyList<LaunchTarget> All()
        {
            ...
            List<LaunchTarget> list = [.. BuiltIns, .. ScanStartMenu()];
            _cached = [.. list
        ...
        public static bool TryResolveLabel(string? labelOrTarget, out string target, out string displayName)
        {
            target = "";
            displayName = "";
            if (string.IsNullOrWhiteSpace(labelOrTarget))
            {
                return false;
            }

            string text = labelOrTarget.Trim();

            foreach (LaunchTarget t in All())
            {
                string label = $"{t.DisplayName}  ({t.Target})";
        ...
        {
            return All().Select(t => $"{t.DisplayName}  ({t.Target})");
        }
        ...
        {
            string q = query?.Trim() ?? "";
            IReadOnlyList<LaunchTarget> all = All();
        ...
        private static IEnumerable<LaunchTarget> ScanStartMenu()
        {
            ...
            foreach (string? root in roots.Where(Directory.Exists))
            {
                ...
                foreach (string? file in files.Take(400))
                {
                    string name;
